...
internal sealed class VaccineRepository(AppDbContext dbContext) : IVaccineRepository
{
    private static readonly IReadOnlyDictionary<string, LambdaExpression> _fieldSelectors =
        new Dictionary<string, LambdaExpression>(StringComparer.OrdinalIgnoreCase)
        {
            ...
            ["createdAtUtc"] = (Expression<Func<Vaccine, DateTimeOffset>>)(v => v.CreatedAt),
            ["createdBy"] = (Expression<Func<Vaccine, string>>)(v => v.CreatedBy),
            ["updatedAt"] = (Expression<Func<Vaccine, DateTimeOffset?>>)(v => v.UpdatedAt),
            ["updatedAtUtc"] = (Expression<Func<Vaccine, DateTimeOffset?>>)(v => v.UpdatedAt),
    ...
    public async Task<IReadOnlyList<GetVaccineResponse>> GetResponsesAsync(SearchParameters? searchParameters = null, CancellationToken cancellationToken = default)
    {
        IQueryable<Vaccine> query = dbContext.Vaccines.AsNoTracking();
        query = SearchQueryHelper.Apply(query, searchParameters, _fieldSelectors, defaultSortField: "name");
