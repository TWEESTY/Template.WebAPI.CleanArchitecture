...
public static class SearchQueryHelper
{
        ...
        string? defaultSortField)
    {
        ...
        foreach (SearchParameters.GroupCriteria group in groupCriteria ?? Enumerable.Empty<SearchParameters.GroupCriteria>())
        {
        ...
        foreach (SearchParameters.SortCriteria sort in sortCriteria ?? Enumerable.Empty<SearchParameters.SortCriteria>())
        {
    ...
    private static Expression<Func<T, bool>>? BuildFilterCollectionExpression<T>(
        SearchParameters.FilterCriteriaCollection collection,
        IReadOnlyDictionary<string, LambdaExpression> fieldSelectors)
    {
